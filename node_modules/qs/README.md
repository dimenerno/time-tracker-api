...
assert.deepEqual(nullObject, { a: { hasOwnProperty: 'b' } ***REMOVED***;
```
...
assert.deepEqual(protoObject, { a: { hasOwnProperty: 'b' } ***REMOVED***;
```
...
assert.deepEqual(deep, { a: { b: { '[c][d][e][f][g][h][i]': 'j' } } ***REMOVED***;
```
...
assert.deepEqual(withDots, { a: { b: 'c' } ***REMOVED***;
```
...
assert.deepEqual(withMaxIndex, { a: { '100': 'b' } ***REMOVED***;
```
...
assert.deepEqual(withArrayLimit, { a: { '1': 'b' } ***REMOVED***;
```
...
assert.deepEqual(noParsingArrays, { a: { '0': 'b' } ***REMOVED***;
```
...
assert.deepEqual(mixedNotation, { a: { '0': 'b', b: 'c' } ***REMOVED***;
```
...
assert.equal(qs.stringify({ a: { b: 'c' } ***REMOVED***, 'a%5Bb%5D=c');
```

This encoding can be disabled by setting the `encode` option to `false`:

```javascript
var unencoded = qs.stringify({ a: { b: 'c' } }, { encode: false ***REMOVED***;
...
```javascript
qs.stringify({ a: ['b', 'c', 'd'] }, { indices: false ***REMOVED***;
// 'a=b&a=c&a=d'
```
...
// 'a[]=b&a[]=c'
qs.stringify({ a: ['b', 'c'] }, { arrayFormat: 'repeat' ***REMOVED***
// 'a=b&a=c'
qs.stringify({ a: ['b', 'c'] }, { arrayFormat: 'comma' ***REMOVED***
// 'a=b,c'
```
...
```javascript
qs.stringify({ a: { b: { c: 'd', e: 'f' } } ***REMOVED***;
// 'a[b][c]=d&a[b][e]=f'
```
...
```javascript
qs.stringify({ a: { b: { c: 'd', e: 'f' } } }, { allowDots: true ***REMOVED***;
// 'a.b.c=d&a.b.e=f'
```
...
assert.equal(qs.stringify({ a: {} ***REMOVED***, '');
assert.equal(qs.stringify({ a: [{}] ***REMOVED***, '');
assert.equal(qs.stringify({ a: { b: []} ***REMOVED***, '');
assert.equal(qs.stringify({ a: { b: {}} ***REMOVED***, '');
```
...
```javascript
assert.equal(qs.stringify({ a: 'b', c: 'd' }, { addQueryPrefix: true ***REMOVED***, '?a=b&c=d');
```

The delimiter may be overridden with stringify as well:

```javascript
assert.equal(qs.stringify({ a: 'b', c: 'd' }, { delimiter: ';' ***REMOVED***, 'a=b;c=d');
```
...
assert.equal(
    qs.stringify({ a: date }, { serializeDate: function (d) { return d.getTime(); } ***REMOVED***,
    'a=7'
);
```
...
assert.equal(qs.stringify({ a: 'c', z: 'y', b : 'f' }, { sort: alphabeticalSort ***REMOVED***, 'a=c&b=f&z=y');
```
...
qs.stringify({ a: 'b', c: 'd', e: { f: new Date(123), g: [2] } }, { filter: filterFunc ***REMOVED***;
// 'a=b&c=d&e[f]=123&e[g][0]=4'
qs.stringify({ a: 'b', c: 'd', e: 'f' }, { filter: ['a', 'e'] ***REMOVED***;
// 'a=b&e=f'
qs.stringify({ a: ['b', 'c', 'd'], e: 'f' }, { filter: ['a', 0, 2] ***REMOVED***;
// 'a[0]=b&a[2]=d'
```
...
```javascript
var strictNull = qs.stringify({ a: null, b: '' }, { strictNullHandling: true ***REMOVED***;
...
```javascript
var nullsSkipped = qs.stringify({ a: 'b', c: null}, { skipNulls: true ***REMOVED***;
...
```javascript
var iso = qs.stringify({ æ: 'æ' }, { charset: 'iso-8859-1' ***REMOVED***;
...
```javascript
var numeric = qs.stringify({ a: '☺' }, { charset: 'iso-8859-1' ***REMOVED***;
...
```javascript
var sentinel = qs.stringify({ a: '☺' }, { charsetSentinel: true ***REMOVED***;
assert.equal(sentinel, 'utf8=%E2%9C%93&a=%E2%98%BA');

var isoSentinel = qs.stringify({ a: 'æ' }, { charsetSentinel: true, charset: 'iso-8859-1' ***REMOVED***;
...
var shiftJISEncoded = qs.stringify({ a: 'こんにちは！' }, { encoder: encoder ***REMOVED***;
...
assert.equal(qs.stringify({ a: 'b c' }, { format : 'RFC3986' ***REMOVED***, 'a=b%20c');
assert.equal(qs.stringify({ a: 'b c' }, { format : 'RFC1738' ***REMOVED***, 'a=b+c');
```
